...
namespace Vars
{
	...
	NAMESPACE_BEGIN(Menu)
		...
		CVarEnum(Indicators, 0b00000, NONE, Ticks = 1 << 0, CritHack = 1 << 1, Spectators = 1 << 2, Ping = 1 << 3, Conditions = 1 << 4, SeedPrediction = 1 << 5, NavBot = 1 << 6)
		...
		CVar(NavBotDisplay, DragBox_t(), NOBIND)
	...
	NAMESPACE_BEGIN(Colors)
		...
		CVar(NoSpread, Color_t(75, 175, 255, 255), VISUAL)
		...
		CVar(NavbotPath, Color_t(255, 255, 0, 255), VISUAL)
		CVar(NavbotArea, Color_t(0, 255, 0, 255), VISUAL)
		CVar(NavbotBlacklist, Color_t(255, 0, 0, 255), VISUAL)
	...
	NAMESPACE_BEGIN(Aimbot)
		...
		SUBNAMESPACE_BEGIN(Projectile)
			...
			CVarEnum(AutoDetonate, 0b00, NONE, Stickies = 1 << 0, Flares = 1 << 1, MaxDamage = 1 << 2, PreventSelfDamage = 1 << 3)
			...
			CVar(GroundMaxChanges, 2, DEBUGVAR)
			CVar(GroundMaxChangeTime, 30, DEBUGVAR)
			...
			CVar(AutodetAccountPing, true, DEBUGVAR)
		...
		SUBNAMESPACE_BEGIN(Melee)
			...
			SUBNAMESPACE_BEGIN(AutoEngie)
				CVarEnum(AutoRepair, 0b111, NONE, Sentry = 1 << 0, Dispenser = 1 << 1, Teleporter = 1 << 2)
				CVarEnum(AutoRepairPrio, 0, NONE, Off, Sentry, Dispenser, Teleporter)
				
				CVarEnum(AutoUpgrade, 0b111, NONE, Sentry = 1 << 0, Dispenser = 1 << 1, Teleporter = 1 << 2)
				CVar(AutoUpgradeSentryLVL, 3)
				CVar(AutoUpgradeDispenserLVL, 3)
				CVar(AutoUpgradeTeleporterLVL, 2)
	...
	NAMESPACE_BEGIN(AntiHack)
		SUBNAMESPACE_BEGIN(AntiAim)
			CVar(Enabled, false)
			CVarEnum(PitchReal, 0, NONE, None, Up, Down, Zero, Jitter, ReverseJitter, Flip, BIGRANDOM)
			CVarEnum(PitchFake, 0, NONE, None, Up, Down, Jitter, ReverseJitter, Flip, BIGRANDOM)
			Enum(Yaw, Forward, Left, Right, Backwards, Edge, Jitter, Spin, BIGRANDOM)
			CVar(YawReal, 0)
			CVar(YawFake, 0)
	...
	NAMESPACE_BEGIN(Visuals)
		...
		SUBNAMESPACE_BEGIN(Misc)
			...
			CVar(KillstreakWeapons, false, VISUAL)
	...
	NAMESPACE_BEGIN(Misc)
		SUBNAMESPACE_BEGIN(Movement)
			...
			SUBNAMESPACE_BEGIN(NavEngine)
				CVar(Enabled, false)
				CVarEnum(Draw, 0b0000011, VISUAL, Path = 1 << 0, Area = 1 << 1, Blacklist = 1 << 2)
				CVarEnum(LookAtPath, 0, NONE, Off, Plain, Silent)
			
				CVar(PathInSetup, false, NOSAVE) // debug
				CVar(SafePathing, false, NOSAVE) // debug
				CVar(StickyIgnoreTime, 15, NOSAVE) // debug
				CVar(StuckDetectTime, 2, NOSAVE) // debug
				CVar(StuckBlacklistTime, 120, NOSAVE) // debug
				CVar(StuckExpireTime, 5, NOSAVE) // debug
				CVar(StuckTime, 0.2f, NOSAVE) // debug

				CVar(VischeckEnabled, false, NOSAVE) // debug
				CVar(VischeckTime, 2.f, NOSAVE) // debug
				CVar(VischeckCacheTime, 240, NOSAVE) // debug
			SUBNAMESPACE_END(NavEngine)

			SUBNAMESPACE_BEGIN(NavBot)
				CVar(Enabled, false)
				CVarEnum(Blacklist, 0b0101111, NONE, NormalThreats = 1 << 0, DormantThreats = 1 << 1, Players = 1 << 2, Stickies = 1 << 3, Projectiles = 1 << 4, Sentries = 1 << 5)
				CVar(BlacklistDelay, 0.5f)
				CVar(BlacklistDormantDelay, 1.f)
				CVar(BlacklistSlightDangerLimit, 2)
				CVarEnum(RechargeDT, 0, NONE, Off, Always, WaitForFL)
				CVar(RechargeDTDelay, 5)
				CVarEnum(AutoScope, 0, NONE, Off, Simple, MoveSim)
				CVar(AutoScopeCancelTime, 3)
				CVarEnum(Preferences, 0b0, NONE, SearchHealth = 1 << 0, SearchAmmo = 1 << 1, ReloadWeapons = 1 << 2, StalkEnemies = 1 << 3, DefendObjectives = 1 << 4, CaptureObjectives = 1 << 5, HelpCaptureObjectives = 1 << 6, EscapeDanger = 1 << 7, SafeCapping = 1 << 8, TargetSentries = 1 << 9, AutoEngie = 1 << 10, TargetSentriesLowRange = 1 << 11, HelpFriendlyCaptureObjectives = 1 << 12, DontEscapeDangerIntel = 1 << 13)
				CVarEnum(WeaponSlot, 0, NONE, Off, Best, Primary, Secondary, PDA, PDA2, Melee)

				CVar(StickyDangerRange, 600, NOSAVE) // debug
				CVar(ProjectileDangerRange, 600, NOSAVE) // debug
				CVar(AutoScopeUseCachedResults, true, NOSAVE) // debug
