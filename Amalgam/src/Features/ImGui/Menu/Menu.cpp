	...
	case 0:
		if (BeginTable("AimbotTable", 2))
		{
			...
			if (Section("Projectile"))
			{
				...
				FDropdown("Auto detonate", Vars::Aimbot::Projectile::AutoDetonate, { "Stickies", "Flares", "##Divider", "Damage priority", "Prevent self damage" }, {}, FDropdown_Multi | FDropdown_Left );
			...
			if (Vars::Debug::Options.Value)
			{
				if (Section("Debug## Projectile", true))
				{
					...
					FToggle("Auto detonate predict lag", Vars::Aimbot::Projectile::AutodetAccountPing);
				} EndSection();
			}
			...
			if (Section("Auto engie", true))
			{
				FDropdown( "Auto repair", Vars::Aimbot::Melee::AutoEngie::AutoRepair, { "Sentrygun","Dispenser", "Teleporter" }, {}, FDropdown_Multi);
				FDropdown( "Auto upgrade", Vars::Aimbot::Melee::AutoEngie::AutoUpgrade, { "Sentrygun", "Dispenser", "Teleporter" }, {}, FDropdown_Multi );
				FDropdown( "Repair priority", Vars::Aimbot::Melee::AutoEngie::AutoRepairPrio, { "None", "Sentrygun", "Dispenser", "Teleporter" });
				FSlider( "Sentry LVL", Vars::Aimbot::Melee::AutoEngie::AutoUpgradeSentryLVL, 1, 3 );
				FSlider( "Dispenser LVL", Vars::Aimbot::Melee::AutoEngie::AutoUpgradeDispenserLVL, 1, 3 );
				FSlider( "Teleporter LVL", Vars::Aimbot::Melee::AutoEngie::AutoUpgradeTeleporterLVL, 1, 3 );
			} EndSection();
...
void CMenu::MenuVisuals(int iTab)
{
	...
	case 0:
		if (BeginTable("VisualsESPTable", 2))
		{
			...
			{
				FToggle("Distance to alpha", Vars::ESP::Dist2Alpha);
				FSlider("Max render distance", Vars::ESP::MaxDist, 512, 4096, 128, "%i", FSlider_Clamp);
	...
	case 2:
		if (BeginTable("VisualsGlowTable", 2))
		{
			...
			if (Section("Friendly", true))
			{
				...
				FToggle("Distance to alpha## Friendly", Vars::Glow::Friendly::Dist2Alpha, FToggle_Left, nullptr, "Distance to alpha friendly glow");
				FSlider("Max render distance## Friendly", Vars::Glow::Friendly::MaxDist, 512, 4096, 128, "%i", FSlider_Clamp, nullptr, "Max render dist friendly glow");
				FSlider("Min render distance## Friendly", Vars::Glow::Friendly::MinDist, 0, 2048, 128, "%i", FSlider_Clamp, nullptr, "Min render dist friendly glow");
			...
			if (Section("Enemy", true))
			{
				...
				FToggle("Distance to alpha## Enemy", Vars::Glow::Enemy::Dist2Alpha, FToggle_Left, nullptr, "Distance to alpha enemy glow");
				FSlider("Max render distance## Enemy", Vars::Glow::Enemy::MaxDist, 512, 4096, 128, "%i", FSlider_Clamp, nullptr, "Max render dist enemy glow");
				FSlider("Min render distance## Enemy", Vars::Glow::Enemy::MinDist, 0, 2048, 128, "%i", FSlider_Clamp, nullptr, "Min render dist enemy glow");
			...
			if (Section("World", true))
			{
				...
				FToggle("Distance to alpha## World", Vars::Glow::World::Dist2Alpha, FToggle_Left, nullptr, "Distance to alpha world glow");
				FSlider("Max render distance## World", Vars::Glow::World::MaxDist, 512, 4096, 128, "%i", FSlider_Clamp, nullptr, "Max render dist world glow");
				FSlider("Min render distance## World", Vars::Glow::World::MinDist, 0, 2048, 128, "%i", FSlider_Clamp, nullptr, "Min render dist world glow");
	...
	case 3:
		if (BeginTable("VisualsMiscTable", 2))
		{
			...
			if (Section("Thirdperson", true))
			{
				...
				FToggle("Thirdperson collision", Vars::Visuals::ThirdPerson::Collision, FToggle_Left);
	...
	case 5:
		if (BeginTable("MenuTable", 2))
		{
			...
			{
				FDropdown("Indicators", Vars::Menu::Indicators, { "Ticks", "Crit hack", "Spectators", "Ping", "Conditions", "Seed prediction", "Nav bot"}, {}, FDropdown_Multi );
	...
	case 0:
		if (BeginTable("MiscTable", 2))
		{
			...
			if (Section("Exploits", true))
			{
				...
				{
					FSlider("Ping", Vars::Misc::Exploits::PingTarget, 1, 100, 1, "%i", FSlider_Right | FSlider_Clamp);
				}
				PopTransparent();
				FToggle("Equip region unlock", Vars::Misc::Exploits::EquipRegionUnlock, FToggle_Left);
				FToggle("Break shoot sound", Vars::Misc::Exploits::BreakShootSound, FToggle_Right); 
			...
			{
				FDropdown("Anti-backstab", Vars::Misc::Automation::AntiBackstab, { "Off", "Yaw", "Pitch", "Fake" }, {}, FDropdown_Left); // pitch/fake _might_ slip up some auto backstabs
				FDropdown("Autojoin class", Vars::Misc::Automation::ForceClass, { "Off", "Scout", "Soldier", "Pyro", "Demoman", "Heavy", "Engineer", "Medic", "Sniper", "Spy" }, {0,1,3,7,4,6,9,5,2,8}, FDropdown_Right);
			...
			if (Section("Game", true))
			{
				...
				FToggle("Valve Allows Cheats", Vars::Misc::Game::VACBypass, FToggle_Left);
				FToggle("Anti cheat compatibility", Vars::Misc::Game::AntiCheatCompatibility, FToggle_Right);
			} EndSection();
			if (Vars::Debug::Options.Value)
			{
				...
				PushTransparent(!FGet(Vars::Misc::Movement::NavEngine::Enabled));
				{
					...
					PushTransparent(!FGet(Vars::Misc::Movement::NavBot::Enabled) || !FGet(Vars::Misc::Movement::NavEngine::Enabled));
					{
						...
						PushTransparent(!(FGet(Vars::Misc::Movement::NavBot::Blacklist) & Vars::Misc::Movement::NavBot::BlacklistEnum::NormalThreats)
										...
										|| !FGet(Vars::Misc::Movement::NavBot::Enabled) || !FGet(Vars::Misc::Movement::NavEngine::Enabled));
						{
...
#endif
				FToggle("Load debug settings", Vars::Config::LoadDebugSettings, FToggle_Left); FTooltip("Load/Save debug settings in this config");
			} EndSection();
			if (Section("Extra"))
			{
...
void CMenu::DrawBinds()
{
	...
	std::function<void(int)> getBinds = [&](int iParent)
		{
			for (auto it = F::Binds.m_vBinds.begin(); it < F::Binds.m_vBinds.end(); it++)
			{
				...
				if (tBind.m_bVisible || m_bIsOpen)
				{
					...
					vInfo.emplace_back(tBind.m_bActive, tBind.m_sName.c_str(), sType, sInfo, iBind, tBind);
				}
	...
	PushFont(F::Render.FontSmall);
	for (auto& [_, sName, sInfo, sState, iBind, tBind] : vInfo)
	{
	...
	if (Begin("Binds", nullptr, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_AlwaysAutoResize))
	{
		...
		PushFont(F::Render.FontSmall);
		int i = 0; for (auto& [bActive, sName, sInfo, sState, iBind, tBind] : vInfo)
		{
			float flPosX = 0;
