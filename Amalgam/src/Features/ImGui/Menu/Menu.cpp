		...
		if (BeginTable("MiscTable", 2))
		{
			...
			if (Section("Navbot", true))
			{
				PushTransparent(!FGet(Vars::Misc::Movement::NavEngine::Enabled));
				{
					...
					PushTransparent(!FGet(Vars::Misc::Movement::NavBot::Enabled) || !FGet(Vars::Misc::Movement::NavEngine::Enabled));
					{
						...
						PopTransparent();
						FDropdown("Auto scope", Vars::Misc::Movement::NavBot::AutoScope, {"Off", "Simple", "Movesim"});
			...
			if (Vars::Debug::Options.Value)
			{
				if (Section("Debug## Navbot", true))
				{
					...
					FToggle("Auto scope use cached results", Vars::Misc::Movement::NavBot::AutoScopeUseCachedResults);
